...
---@field detector BitForge.AzerothPrime.Control.Detector
---@field openScanner BitForge.AzerothPrime.Control.OpenScanner
---@field inventory BitForge.AzerothPrime.Control.Inventory
---@field recipes BitForge.AzerothPrime.Control.Recipes
---@field deposit BitForge.AzerothPrime.Control.Deposit
---@field adapters BitForge.AzerothPrime.Control.Adapters
---@field sellScanner BitForge.AzerothPrime.Control.SellScanner
---@field seller BitForge.AzerothPrime.Control.Seller
---@field disenchantProbe BitForge.AzerothPrime.Control.DisenchantProbe
...
---@field allowListAudit BitForge.AzerothPrime.Control.AllowListAudit
-- Nilable: a release build ships no debug/lines.lua at all.
---@field debugNotices BitForge.AzerothPrime.Control.DebugNotices|nil
-- Nilable for the same reason: debug/curationReview.lua, whose one call
-- site is control/openScanner.lua's scan.
---@field curationReview BitForge.AzerothPrime.Control.CurationReview|nil
...
-- The four addons this one replaces. A manual installer who leaves one of
-- them in place -- or CurseForge's own client, which does not remove a
-- folder a release stops shipping -- leaves two live copies of every
-- feature, which scan the bags twice and act on them twice. Their saved
-- data is not touched by disabling them -- the adoption simply happens at
-- the next login, once StillInstalled stops finding them switched on.
local REPLACED = { "BitForge_Openables", "BitForge_BatchSell", "BitForge_UPS", "BitForge_Dispatch" }

--- Disables every replaced addon still switched on, and names the ones it
--- disabled. Nil when none needed it: either a folder is gone entirely, or
--- it was already off and the player has already been told once.
---
--- IsAddOnLoaded cannot drive this: a disabled addon answers false, so a
--- loaded check would stop the moment the disable below made it true.
--- DoesAddOnExist and GetAddOnEnableState both see a folder regardless of
--- load or enable state (AddOnsDocumentation.lua, both AllowedWhenUntainted).
---
--- GetAddOnEnableState answers three values, not two -- None/Some/All -- and
--- Some counts as enabled: it is the state a per-character disable leaves an
--- addon in on every OTHER character, exactly the state that would make this
--- check meet the same addon again on every alt if it only compared to All.
---
--- Both calls take `character`, defaulted to "0" when omitted. Verified
--- against Blizzard_AddonList/AddonList.lua: its own "All characters" radio
--- (the default selection) resolves to a nil character argument, and DisableAddOn
--- with no character argument uses that same default -- so omitting it here,
--- as below, is already the account-wide disable, not a per-character one.
---@return string[]|nil
local function StillInstalled()
    local found
    for _, name in ipairs(REPLACED) do
        if C_AddOns.DoesAddOnExist(name) then
            if C_AddOns.GetAddOnEnableState(name) ~= Enum.AddOnEnableState.None then
...
local ADOPTS = { "Openables", "UPS", "BatchSell", "Dispatch" }
...
--- tables. The one tool this step needs for the two shapes in AzerothPrime's own
...
---
--- target is always AzerothPrime's own freshly-seeded default for that address:
--- SeedDefaults already deep-copied DB_DEFAULTS into moduleDB.global before
--- this step runs, since AzerothPrime is new for every profile. BatchSell's
--- global.rules and Openables' global.point agree with AzerothPrime's own at
--- every address those two themselves declared, bar the deliberate exceptions
--- tests/test_azerothprime_adopt.lua names one by one -- rules.reagents.expansions
--- is the only one so far, where moving the adopter to AzerothPrime's new default
...
--- AzerothPrime has grown since, or one of those exceptions -- and target already
...
--- any of the three call sites below run, AzerothPrime's own allocation is
--- guaranteed to exist.
---@param charKey string
---@return table
local function AzerothPrimeCharSlot(charKey)
    ---@type table
...
-- The version Dispatch itself last reached under its own name. Frozen at 4
-- regardless of what enum.SCHEMA_VERSION becomes from here: a later version
-- of THIS module has nothing to do with how far a retired Dispatch table
-- got, because no build ever shipped "Dispatch" past schema 4 -- version 5
-- is this module's own adoption step, introduced only after the rename.
local DISPATCH_CURRENT_VERSION = 4

--- Brings a still-below-DISPATCH_CURRENT_VERSION Dispatch table's own global
--- and char data up to that shape, in place, before step 5 below overlays
--- it. Without this, a Dispatch profile that fell behind its own schema --
--- stale on disk since before this module's rename, and never given another
--- login to migrate itself -- would arrive still shaped for a step that
--- exists to fix exactly that shape, and nothing downstream would ever fix
--- it again: step 5 only ever runs once per profile.
---
--- Deliberately not a call into steps[2]-[4] below. Those reach through
--- moduleDB's own live proxy and, for their per-character passes, through
--- AzerothPrimeCharSlot/ADDON_NAME to touch THIS module's own live storage
--- directly -- correct for them, since they run against the module actually
--- starting up, and exactly wrong here: a raw retired table has no live
--- proxy of its own and no ADDON_NAME to resolve through. This mirrors their
--- address-for-address logic instead, operating on raw.global/raw.char
--- directly, the same way step 5 itself has to.
---
--- A player whose Dispatch build was the last one ever shipped under that
--- name reaches this already at version 4, and every branch below is then a
--- no-op for them.
---@param raw table  the raw retired table GetRetiredModuleDB handed back
---@param stored number  raw.global.schemaVersion as it stood before step 5
---   nils it out, or 0 for a table that never stored one
local function AdvanceDispatchShape(raw, stored)
    if stored < 2 then
        raw.global = raw.global or {}
        local globalData = raw.global
        local merged = globalData.itemOverrides or {}
        globalData.itemOverrides = merged

        -- Mirrors step 2's own three passes exactly, blacklist inversion
        -- included -- see that step's own comment for why `== true` and why
        -- owners is copied rather than shared.
        for itemID, blacklisted in pairs(globalData.blacklist or {}) do
            if blacklisted == true then
                MergeOverride(merged, itemID, "open", false)
            end
        end
        for itemID, status in pairs(globalData.list or {}) do
            MergeOverride(merged, itemID, "sell", status)
        end
        for itemID, override in pairs(globalData.overrides or {}) do
        ...
        for _, charData in pairs(raw.char or {}) do
            charData.itemOverrides = charData.itemOverrides or {}
            for itemID, status in pairs(charData.list or {}) do
                MergeOverride(charData.itemOverrides, itemID, "sell", status)
            end
        end
    end
    ...
    if stored < 3 then
        if raw.global then
            raw.global.blacklist = nil
            raw.global.list = nil
            raw.global.overrides = nil
        end
        for _, charData in pairs(raw.char or {}) do
            charData.list = nil
        end
...
local function onPlayerReady()
    ...
    if stillInstalled then
        ...
        -- Neither UpgradeModuleDB nor its adoption steps run this session.
        -- Stamping the schema now, with a retired module StillInstalled just
        -- disabled still loaded, would mark this profile as already adopted
        -- against a login where GetRetiredModuleDB refused every one of its
        -- keys -- a version stamped on a session that could not adopt looks
        -- exactly like one that ran cleanly. Next login the disabled addon
        -- is not loaded, GetRetiredModuleDB answers, and adoption runs for
        -- the first time.
        return
    end
    ...
    BitForge:UpgradeModuleDB(ADDON_NAME, {
        ...
        steps   = {
            ...
            [1] = function(moduleDB)
                ...
                if openables then
                    ...
                    for charKey, charData in pairs(openables.char or {}) do
                        if charData.professionRanks then
                            AzerothPrimeCharSlot(charKey).professionRanks = charData.professionRanks
                        end
                    end
                ...
                if ups then
                    ...
                    for charKey, charData in pairs(ups.char or {}) do
                        local slot = AzerothPrimeCharSlot(charKey)
                ...
                if batchSell then
                    ...
                    for charKey, charData in pairs(batchSell.char or {}) do
                        if charData.list then
                            AzerothPrimeCharSlot(charKey).list = charData.list
                        end
                    end
            ...
            [2] = function(moduleDB)
                local azerothPrimeGlobal = moduleDB.global
                local merged = azerothPrimeGlobal.itemOverrides
                ...
                for itemID, blacklisted in pairs(azerothPrimeGlobal.blacklist or {}) do
                    if blacklisted == true then
                        MergeOverride(merged, itemID, "open", false)
                    end
                end

                for itemID, status in pairs(azerothPrimeGlobal.list or {}) do
                    MergeOverride(merged, itemID, "sell", status)
                end
                ...
                for itemID, override in pairs(azerothPrimeGlobal.overrides or {}) do
                    if type(override) == "table" then
                        MergeOverride(merged, itemID, "bank", override.dest)
                        MergeOverride(merged, itemID, "owners",
                            override.owners and CopyTable(override.owners))
                ...
                -- AzerothPrimeCharSlot for one of those would create an AzerothPrime
            ...
            [3] = function(moduleDB)
                local azerothPrimeGlobal = moduleDB.global
                azerothPrimeGlobal.blacklist = nil
                azerothPrimeGlobal.list = nil
                azerothPrimeGlobal.overrides = nil
                ...
                for _, charKey in ipairs(BitForge:GetModuleCharKeys(ADDON_NAME)) do
                    AzerothPrimeCharSlot(charKey).list = nil
                end
            end,
            ...
            -- Dispatch is this same module under its own retired name --
            -- Task 2's rename touched no behavior, so a Dispatch profile
            -- moves across as a whole table rather than an address this
            -- step has to translate field by field the way step 1 does for
            -- Openables/UPS/BatchSell: global and every char slot, not
            -- merely the character logging in right now (AzerothPrimeCharSlot
            -- is what reaches every alt; see its own doc comment).
            --
            -- Not necessarily already this module's current shape, though:
            -- Dispatch's own folder can be stale on disk for a long time
            -- before Step 0 disables it (its own code stops getting updates
            -- the moment a release stops shipping it), so its stored
            -- schemaVersion can be anywhere from 0 to DISPATCH_CURRENT_VERSION.
            -- AdvanceDispatchShape closes that gap in place, address for
            -- address, before the overlay below -- without it, a profile
            -- whose per-item decisions still lived in the pre-merge
            -- blacklist/list/overrides keys would land in those addresses
            -- unread by anything, which is silent data loss dressed as a
            -- successful adoption.
            --
            -- A stored version ABOVE DISPATCH_CURRENT_VERSION is asserted
            -- against instead of assumed: no build has ever shipped
            -- "Dispatch" past schema 4, so a table claiming otherwise is a
            -- shape nothing here has ever seen, and overlaying it on trust
            -- would be the same mistake in the other direction. Refused
            -- rather than adopted -- reported for a developer to look at,
            -- left in place for a next build to make sense of, and this
            -- module starts on its own defaults for this session.
            --
            -- schemaVersion is the one field this step must not carry --
            -- it is Dispatch's own history, not this module's, and
            -- UpgradeModuleDB stamps this module's real version the moment
            -- this step returns. In the live client that stamp would
            -- overwrite a carried value anyway, but a test driving this
            -- step directly, the way this file's sibling schema tests do
            -- for the other three, would not get that overwrite for free.
            --
            -- Protected the same way steps 1-4 are: not by comparing
            -- against what is already stored, but by never running at all
            -- once this module's own schemaVersion has reached spec.version
            -- (model.ResolveSchemaUpgrade's "current" verdict) -- so a
            -- Dispatch table that resurfaces after this step has already
            -- run once, from some anomaly rather than an existing player's
            -- normal upgrade path, is never read again.
            --
            -- One-way: DropRetiredModuleDB below means a profile this step
            -- has adopted can never be read by a build that still calls
            -- itself Dispatch.
            [5] = function(moduleDB)
                local dispatch = BitForge:GetRetiredModuleDB("Dispatch")
                if not dispatch then return end

                local stored = (dispatch.global and dispatch.global.schemaVersion) or 0
                if stored > DISPATCH_CURRENT_VERSION then
                    CallErrorHandler(("BitForge_AzerothPrime: the retired Dispatch profile is stored at"
                        .. " schema %d, newer than any build ever shipped under that name (%d)."
                        .. " Left untouched rather than adopted."):format(stored, DISPATCH_CURRENT_VERSION))
                    return
                end

                AdvanceDispatchShape(dispatch, stored)

                if dispatch.global then
                    dispatch.global.schemaVersion = nil
                    Overlay(moduleDB.global, dispatch.global)
                end

                for charKey, charData in pairs(dispatch.char or {}) do
                    Overlay(AzerothPrimeCharSlot(charKey), charData)
                end

                BitForge:DropRetiredModuleDB("Dispatch")
            end,
        },
    }, startModule)
end
...
-- it, and debug/dumps.lua's /bfdump azerothprime open renders through the same
...
-- azerothprime <itemID> answers the same question through the arbiter and is the
...
do
    ...
    -- /bfdump azerothprime allowlist reports too: two commands asking the same
    ...
    --- debug/dumps.lua's /bfdump azerothprime open differ only in how they resolve
    ...
    function control.OpenReportText(bag, slot, itemID)
        ...
        local lines = {
            "BitForge AzerothPrime -- classification report",
        ...
        -- /bfdump azerothprime <itemID>, which does answer this through the
...
-- gesture that shows it, and debug/dumps.lua's /bfdump azerothprime sell renders
...
do
    ...
    --- debug/dumps.lua's /bfdump azerothprime sell takes an item ID. BuildDump has
    ...
    function control.RenderSellReport(report)
        ...
        local lines = {
            "BitForge AzerothPrime -- sell verdict report",
            BitForge:ReportHeader(ADDON_NAME),
            "",
        }
