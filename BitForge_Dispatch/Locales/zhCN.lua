...
L["msg:replacedInstalled"] = "Dispatch：%s 仍已安装。"
L["msg:replacedInstalledFix"] = "删除它，然后重新登录，让 Dispatch 接手。"
...
L["report:blurbField"] = "这份报告包含上次扫描排出的每一个候选物品，按排名顺序列出：各自的名称、物品 ID、背包与格子、堆叠数量、优先级以及排到该位置的原因，它是否是需要钥匙的上锁箱子、它所在的格子当前是否被锁定，以及它是否正在冷却或被推迟。这里不会写出你角色的名字、所在服务器、公会或阵营，也不包含任何物品的提示文字。"

L["report:blurbAllowList"] = "这份报告包含插件自己手工维护的两份开启名单 —— 开启白名单与开启黑名单 —— 中的每一件物品，两份名单各占一节；指定其中一份运行时只写这一节。每一行写有物品 ID 与名称、忽略该名单后开启规则得出的判定、作出该判定的那一层规则、这一层给出的优先级，以及仅白名单才有的、名单为它锁定的优先级，还有该条目所属的分组。若某一行写的是该物品的详细信息或提示文字始终没有返回，那说的是你运行命令时这个客户端的缓存，而不是物品本身；首次运行时白名单中的绝大多数都会这样写。它按物品 ID 读取随插件附带的名单和每件物品的提示文字，因此不会描述你背包里的东西 —— 但名单中被你排除或在本次登录期间跳过的物品会被如实标出，取决于本角色状态的判定也会标出，例如你能否使用该物品、能否打开上锁的箱子。这里不会写出你角色的名字、所在服务器、公会或阵营。"
...
L["msg:nothingToDo"] = "Dispatch：没有需要移动的物品。"
L["msg:done"] = "Dispatch：完成。已移动 %d 件物品。"
L["msg:noVacancy"] = "Dispatch：战团银行已满。"
L["msg:blockedCombat"] = "Dispatch：已停止 — 你正在战斗中。"
L["msg:blockedBankClosed"] = "Dispatch：已停止 — 银行已关闭。"
L["msg:blockedCursor"] = "Dispatch：已停止 — 光标上有物品。"
L["msg:blockedLocked"] = "Dispatch：已停止 — 物品已锁定。"
L["msg:moveFailed"] = "Dispatch：已停止 — 移动未能完成。"
L["msg:openProfession"] = "Dispatch：请打开一次%s窗口，以便 Dispatch 记录你已学会的配方。"
...
L["msg:noVacancyPrivate"] = "Dispatch：你的银行已满。"
...
"在战斗中和副本里，游戏有时不会透露物品的详细信息。Dispatch 会保留这些物品，而不是去猜测，所以清单中可能会缺少个别物品 -- 这是正常现象。如果某个判定因为其他原因看起来有问题，值得反馈。"
...
L["rule:junkSub"] = "默认关闭"
L["rule:junkBlurb"] =
"灰色物品，无论具体是哪种物品。默认关闭，因为这通常由其他插件负责处理。如果没有其他插件处理，打开这个选项，Dispatch 会替你清理它们。"
L["rule:epic"] = "传说品质及以上"
L["rule:epicSub"] = "传说、神器、传家宝"
L["rule:epicBlurb"] =
"永不出售。商人会为这些物品显示价格，随后又拒绝这笔交易，因此 Dispatch 不会把它们列入清单。"
L["rule:reagent"] = "制作材料"
...
L["rule:bagsSub"] = "各种容器"
L["rule:bagsBlurb"] =
"永不出售。携带哪些背包由你自己决定，所以 Dispatch 不会对它们做判定。"
L["rule:gear"] = "武器与护甲"
...
L["rule:noneBlurb"] =
"Dispatch 完全不判定的物品类型：任务物品、钥匙、笼中宠物、雕文、WoW 代币、法术材料、箭矢，以及其他已停用的类别。无论上面的规则如何设置，它们都会留在你的背包里。"
...
L["report:blurbDisenchant"] = "此报告包含客户端当前的法术索敌状态，以及本角色是否能够分解物品。此报告还包含你背包里最多八件可能值得分解的武器和护甲，每件都附带它的背包与格子、物品 ID、名称、品质、物品类型，以及 BitForge 自己对该物品是否能够分解所做的预测，另加完整的提示文字。对于品质无法读取的其他任何背包物品，此报告也会说明该物品的背包、格子、物品 ID 和名称。这里不会写出你角色的名字、所在服务器、公会或阵营。"
L["report:blurbDispatch"] = "此报告包含物品的链接和品质、当你携带该物品时它是从哪个背包和栏位得到答案的、每条规则路径为该物品得出的判定 -- 它自身附带的额外细节，以及它是否来自已保存的例外设置 -- 以及各条路径自身的主张、强度和理由，包括其中任何完全无法作答的路径。当物品品质为粗糙时，本报告还会说明，根据你自己的出售设置和垃圾规则设置，暴雪自身的“出售垃圾”功能是否仍会将其卖出。当物品是制作材料时，本报告还会写明随插件附带的材料表把它列为哪些专业的材料、此账号已记录的专业 -- 材料为灵魂绑定时则是当前角色的专业 -- 材料所属的资料片以及你是否勾选了该资料片、你标记过的配方是否需要它，以及材料规则本身以其中哪一项作答 -- 那是材料规则自己的判定，并不总是决定该物品去向的那一条。它始终会列出你标记过的配方，写明其 ID，游戏还能给出名称时也写明名称，因此它写出的是你制作什么，而不只是你随身携带什么。启用诊断时，本报告还会写明你当时所在的地图以及包含它的上级地图；如果该物品被限定在特定地点，还会写明该限定所列出的地点，以及其中哪一个与你当时所在之处相符。物品链接会写明你角色的等级和专精 -- 这是链接自身格式的一部分，去掉它就会丢失让报告可以重现的细节。这里不会写出你角色的名字、所在服务器、公会或阵营。"
