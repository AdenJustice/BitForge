...
L["msg:replacedInstalled"] = "Dispatch: 다음이 아직 설치되어 있습니다: %s"
L["msg:replacedInstalledFix"] = "삭제한 뒤 다시 로그인하면 Dispatch가 넘겨받습니다."
...
L["report:blurbField"] = "이 신고에는 마지막 스캔이 다음에 열 후보로 순위 매긴 모든 항목이 순위 순서대로 담깁니다: 각 항목의 이름, 아이템 ID, 가방과 칸, 스택 수, 우선순위와 그 순위를 받은 이유, 열쇠가 필요한 상자인지 여부, 칸이 현재 잠겨 있는지 여부, 그리고 재사용 대기·보류 여부가 포함됩니다. 캐릭터, 서버, 길드, 진영의 이름은 여기에 없으며, 어떤 아이템의 툴팁 텍스트도 포함되지 않습니다."

L["report:blurbAllowList"] = "이 신고에는 애드온이 직접 목록으로 관리하는 두 개봉 목록, 곧 개봉 허용 목록과 개봉 차단 목록의 모든 아이템이 담깁니다. 목록마다 구획이 하나씩 붙고, 목록을 지정해 실행하면 그 하나만 담깁니다. 각 줄에는 아이템 ID와 이름, 그 목록을 무시했을 때 개봉 규칙이 내리는 판정, 그 판정을 내린 단계, 그 단계가 매긴 우선순위, 그리고 허용 목록에 한해 목록이 고정해 둔 우선순위와 그 항목이 속한 분류가 적힙니다. 아이템 정보나 툴팁이 끝내 오지 않았다고 적힌 줄은 아이템이 아니라 명령을 실행한 시점의 이 클라이언트 캐시를 말합니다. 처음 실행하면 허용 목록 대부분이 그렇게 적힙니다. 함께 배포되는 목록과 각 아이템의 툴팁을 아이템 ID로 읽으므로 가방 속 내용은 설명하지 않습니다 -- 다만 목록에 있는 아이템을 직접 제외했거나 이번 접속 동안 건너뛰었다면 그 사실이, 그리고 이 아이템을 사용할 수 있는지나 잠긴 상자를 열 수 있는지처럼 이 캐릭터의 상태에 따라 갈린 판정이라면 그 점이 함께 적힙니다. 캐릭터, 서버, 길드, 진영의 이름은 여기에 없습니다."
...
L["msg:nothingToDo"] = "Dispatch: 이동할 항목이 없습니다."
L["msg:done"] = "Dispatch: 완료. %d개 아이템을 이동했습니다."
L["msg:noVacancy"] = "Dispatch: 전투부대 은행이 가득 찼습니다."
L["msg:blockedCombat"] = "Dispatch: 중단됨 — 전투 중입니다."
L["msg:blockedBankClosed"] = "Dispatch: 중단됨 — 은행이 닫혔습니다."
L["msg:blockedCursor"] = "Dispatch: 중단됨 — 커서에 무언가 들려 있습니다."
L["msg:blockedLocked"] = "Dispatch: 중단됨 — 아이템이 잠겨 있습니다."
L["msg:moveFailed"] = "Dispatch: 중단됨 — 이동을 완료하지 못했습니다."
L["msg:openProfession"] = "Dispatch: 알고 있는 제조법을 기록할 수 있도록 %s 창을 한 번 열어 주십시오."
...
L["msg:noVacancyPrivate"] = "Dispatch: 개인 은행이 가득 찼습니다."
...
"전투 중이거나 인스턴스 안에서는 게임이 아이템의 세부 정보를 알려주지 않을 때가 있습니다. Dispatch은 이럴 때 추측하는 대신 해당 아이템을 보관하므로, 목록에 몇 개가 빠질 수 있습니다 -- 이는 정상입니다. 그 밖의 이유로 판정이 잘못된 것 같다면 알려주실 만한 가치가 있습니다."
...
L["rule:junkBlurb"] =
"아이템 종류와 상관없이 회색 아이템입니다. 보통 다른 애드온이 이 역할을 맡기 때문에 기본값은 꺼짐입니다. 다른 애드온이 처리하지 않는다면 이 옵션을 켜세요. Dispatch이 대신 정리해 드립니다."
L["rule:epic"] = "전설 등급 이상"
L["rule:epicSub"] = "전설, 유물, 가보"
L["rule:epicBlurb"] =
"절대 판매되지 않습니다. 상인이 가격은 표시하지만 실제로는 거래를 거부하므로, Dispatch은 이 아이템들을 목록에 올리지 않습니다."
...
L["rule:bagsBlurb"] =
"절대 판매되지 않습니다. 어떤 가방을 쓸지는 당신의 선택이므로, Dispatch은 가방을 판정하지 않습니다."
...
L["rule:noneBlurb"] =
"Dispatch이 전혀 판정하지 않는 아이템 종류입니다: 퀘스트 아이템, 열쇠, 우리에 갇힌 애완동물, 문양, WoW 토큰, 주문 재료, 화살, 그리고 그 밖에 퇴역한 분류들입니다. 위 규칙을 어떻게 설정하든 이 아이템들은 가방에 그대로 남습니다."
...
L["report:blurbDisenchant"] = "이 신고에는 클라이언트의 현재 주문 대상 지정 상태와 이 캐릭터가 마력 추출을 할 수 있는지 여부가 담깁니다. 또한 가방 속에서 마력 추출할 가치가 있을 수 있는 무기와 방어구가 최대 여덟 개까지, 각각의 가방과 칸, 아이템 ID, 이름, 품질, 아이템 종류, 그리고 마력 추출이 가능한지에 대한 BitForge 자체의 예측과 함께, 툴팁의 전체 텍스트도 담깁니다. 품질을 읽을 수 없었던 다른 아이템이 있다면 그 아이템의 가방, 칸, 아이템 ID와 이름도 담깁니다. 여기에는 캐릭터, 서버, 길드, 진영 중 어느 것의 이름도 나타나지 않습니다."
L["report:blurbDispatch"] = "이 신고에는 아이템의 링크와 품질, 아이템을 소지하고 있을 때 어느 가방과 칸에서 응답했는지, 각 규칙 경로가 그 아이템에 대해 내린 판정 -- 그 자체의 추가 세부 정보와 저장된 재정의에서 나온 것인지 여부 -- 그리고 각 경로 자신의 주장과 강도와 사유가 담깁니다, 전혀 응답하지 못한 경로가 있다면 그 경로도 포함됩니다. 아이템 품질이 조잡한 등급일 때는 Blizzard의 '쓰레기 아이템 판매' 기능이 판매와 쓰레기 규칙 설정에 따라 이 아이템을 그대로 판매할지 여부도 담깁니다. 아이템이 제작 재료일 때는 함께 배포되는 재료 목록이 그것을 어느 전문기술의 재료로 기재하는지, 이 계정에 기록된 전문기술 -- 귀속된 재료일 때는 이 캐릭터의 전문기술 -- 재료가 속한 확장팩과 그 확장팩을 선택해 두었는지 여부, 지정해 둔 제조법이 그것을 필요로 하는지 여부, 그리고 그중 무엇으로 재료 규칙 자체가 답했는지도 담깁니다. 이는 재료 규칙 자신의 판정이며, 그것이 언제나 이 아이템을 결정한 것은 아닙니다. 지정해 둔 제조법은 언제나 ID로, 게임이 아직 이름을 알려 줄 수 있는 것은 이름까지 함께 나열되므로, 지금 소지한 것뿐 아니라 무엇을 제작하는지도 담기게 됩니다. 진단을 켜 두었을 때는 서 있던 지도와 그 지도를 포함하는 상위 지도도 담기고, 특정 장소에서만 쓸 수 있도록 지정된 아이템일 때는 그렇게 지정된 장소들과 그중 어느 곳이 있던 위치와 일치했는지도 담깁니다. 아이템 링크는 캐릭터의 레벨과 전문화를 담고 있습니다 -- 이는 링크 자체 형식의 일부라서, 이를 빼면 신고를 재현 가능하게 만드는 세부 정보를 잃게 됩니다. 여기에는 캐릭터, 서버, 길드, 진영 중 어느 것의 이름도 나타나지 않습니다."
