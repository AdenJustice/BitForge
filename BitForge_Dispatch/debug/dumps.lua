...
-- Every /bfdump dispatch form, and the one subscription that routes them.
...
-- which is what makes core drop Dispatch's /bfdump line from the roster and
...
-- /bfdump dispatch open [itemID] shows everything detector.Classify reads
...
-- above that function.
--
-- /bfdump dispatch open all shows the other half of the question. A single
...
do
    ...
    local function RenderFieldDump(candidates)
        local lines = {
            "BitForge Dispatch -- ranked field",
    ...
    function control.DumpOpenItem(itemID)
        ...
        if itemID then
            ...
            if not bag then
                BitForge:Print(("Dispatch: item %d is not in your bags"):format(itemID))
                return
            end
        else
            local candidate = openScanner.GetCurrent()
            if not candidate then
                BitForge:Print("Dispatch: nothing on the button to dump")
                return
            end
    ...
    function control.DumpOpenField()
        ...
        if not candidates or #candidates == 0 then
            BitForge:Print("Dispatch: the last scan produced no candidates to rank")
            return
        end
...
-- /bfdump dispatch sell <itemID> captures a whole verdict -- the item, what is
...
do
    ...
    function control.DumpSellItem(itemID)
        if not itemID then
            BitForge:Print("Dispatch: /bfdump dispatch sell <itemID> | disenchant")
            return
        end

        local report = sellScanner.ExplainByID(itemID)
        if not report then
            BitForge:Print(("Dispatch: item %d is not cached yet -- try again in a moment")
                :format(itemID))
            return
        end
    ...
    local function RenderDisenchantReport(scan)
        local lines = {
            "BitForge Dispatch -- disenchant scan",
            BitForge:ReportHeader(ADDON_NAME),
            "",
            scan.state,
        }
...
-- /bfdump dispatch <itemID> answers for all three claimants at once: every
...
-- in this report does, and the eleven report:blurbDispatch footnotes say it is
-- carried "with diagnostics enabled". Deleting that gate puts a location in
-- every player's paste and makes all eleven false. Any other block added here
-- takes the file's rule, not the exception.
do
    ...
    -- does. report:blurbDispatch names it for that reason, in all eleven
    -- locales, as carried only with diagnostics enabled.
    --
    ...
    local function AppendZoneBlock(lines, itemID)
        ...
        -- bearing against an empty entry list -- tests/test_dispatch_zone.lua's
    ...
    local function RenderDispatchReport(facts, verdict, source, settings)
        ...
        local lines = {
            "BitForge Dispatch -- combined verdict",
            BitForge:ReportHeader(ADDON_NAME),
            "",
        }
...
ns:SubscribeCommand(events.MODULE_DUMP, function(addon, argument)
    ...
    BitForge:Print("Dispatch: /bfdump dispatch <itemID> | open [<itemID>|all]"
