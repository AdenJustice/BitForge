...
---@class BitForge.Dispatch.Control.AllowListAudit
...
do
    ...
    for _, bucket in pairs(BUCKET) do
        if not placed[bucket] then
            error("BitForge_Dispatch/debug/allowListAudit.lua: "
...
---@class Dispatch.AuditListSpec
---@field key string  the word /bfdump dispatch allowlist takes to run this one
...
---@field suppress Dispatch.OpenRulesSuppress  the rung this pass switches off
...
---@param spec Dispatch.AuditListSpec
---@param itemID number
...
local function Render(sections)
    local lines = {
        "BitForge Dispatch -- curated open list audit",
...
--- already answer. Reached from /bfdump dispatch allowlist [allow|deny].
---
...
---@param list string|nil  one Dispatch.AuditListSpec key, or nil for both.
...
function allowListAudit.Run(list)
    ...
    -- guarantee.
    assert(#sections > 0, "BitForge Dispatch: the audit matched no curated "
    ...
    local function sweep(attempt)
        ...
        for _, section in ipairs(sections) do
            ...
            assert(#section.rows > 0, "BitForge Dispatch: the audit found no "
