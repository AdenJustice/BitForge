...
local view = ns.view
---@type BitForge.BatchSell.View.RuleDescriptors
...
-- a requirement any more -- an overflow is now reachable rather than clipped --
-- and the window is centred on UIParent, movable and clamped to the screen.
...
---@class BitForge.BatchSell.View.RuleWindow
local ruleWindow = {}
...
-- that reskins neither. UI.CreateCloseButton draws its own themed X rather
-- than three swappable textures, so unlike BitForge_EUI/view/editor.lua's
-- UIPanelCloseButton it takes no facade call of its own (docs/eui-integration.md).
...
function ruleWindow.Toggle()
    ...
    -- the approved wireframe (docs/batchsell-rule-window.html), which ends on
