...
do
    ---@class BitForge.BatchSell.View.MerchantPanel
    local panel = {}
    ...
    --- (model.GetEffectiveStatus), so an item has nine states. One radio group per
    --- scope shows both at once; a single group of five could only ever show one of
    --- them, and a warband entry sitting under a character one -- the case a player
    --- opens this menu to understand -- would be invisible.
    ---
    ...
    function panel.BuildItemMenu(root, item)
        ...
        local function scopeSection(title, scope)
            ...
            local function isSelected(status)
                return (model.GetStatus(itemID, scope) or NO_STATUS) == status
            end

            local function setSelected(status)
                model.SetStatus(itemID, scope, status ~= NO_STATUS and status or nil)
        ...
        root:CreateButton(locale["menu:reportVerdict"], function()
            local text = control.ReportText(item.bagIndex, item.slotIndex)
            if not text then return end
            BitForge:ShowReport(text, locale["report:blurb"])
        end)
    end
    ...
    function panel.Refresh()
        ...
        if count == 0 then
            ...
            frame.status:SetText(format(locale["status:itemsTotal"], count, GetCoinTextureString(total)))
        end
